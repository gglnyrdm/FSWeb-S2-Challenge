...
function cumlelereDonustur(dizi, ayrac = ","){
	...
	const cumleDizi = cumleler.map(cumle => cumle.join(ayrac));
return cumleDizi;
}
console.log(cumlelereDonustur);
...
function paragrafOlustur(dizi,kurCallback,donCallback){
	const paragraf = cumlelereDonustur(cumleler, " ");
	return cumleKur(paragraf[1],paragraf[3],paragraf[5],paragraf[7],paragraf[9]);
}
...
function emojileriDonustur(mesaj,emojiList){
